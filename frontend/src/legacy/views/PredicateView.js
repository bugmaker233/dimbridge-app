...
export default class PredicateView {
    ...
    constructor(data, model, controller, config) {
        ...
        this.config = config;
        this.max_attributes = 20;
        this.last_predicates = null;
    ...
    draw(predicates) {
        ...
        this.last_predicates = predicates;
        let n_boxes = predicates.length;
        let attributes = Object.keys(predicates[0]).slice(0, this.max_attributes);
    ...
    set_max_attributes(value) {
        const parsed = Number(value);
        this.max_attributes = Number.isFinite(parsed)
            ? Math.max(1, Math.min(100, Math.trunc(parsed)))
            : 20;
        if (Array.isArray(this.last_predicates) && this.last_predicates.length > 0) {
            this.draw(this.last_predicates);
        }
    }
}
...
function predicate_single(
    ...
    g_container.draw = function (
        ...
        const updateSelection = () => {
            apply_single_predicate_selection(controller.data, interval_data);
            controller.on_predicate_view_change(controller.data);
        };
        ...
        let drag_max = d3.drag().on("drag", function (event, d) {
            ...
            d.interval_max = clip(new_interval_max, d.interval_min, d.vmax);
            refreshIntervalMarks();
            updateSelection();
        });
        controls_end_circles = g
        ...
        intervals_fg.call(
            d3
                .drag()
                .on("start", (event, d) => {
                    d.drag_anchor_x = event.x;
                })
                .on("drag", (event, d) => {
                    let scale = sx.domain([d.vmin, d.vmax]);
                    let delta =
                        scale.invert(event.x) -
                        scale.invert(d.drag_anchor_x ?? event.x);
                    d.drag_anchor_x = event.x;

                    let width = d.interval_max - d.interval_min;
                    let next_min = clip(d.interval_min + delta, d.vmin, d.vmax);
                    let next_max = next_min + width;
                    if (next_max > d.vmax) {
                        next_max = d.vmax;
                        next_min = d.vmax - width;
                    }
                    if (next_min < d.vmin) {
                        next_min = d.vmin;
                        next_max = d.vmin + width;
                    }

                    d.interval_min = next_min;
                    d.interval_max = next_max;
                    refreshIntervalMarks();
                    updateSelection();
                })
                .on("end", (event, d) => {
                    delete d.drag_anchor_x;
                }),
        );
...
function apply_single_predicate_selection(data, interval_data) {
    data.forEach((datum) => {
        let selected = interval_data.every((interval) => {
            let value = Number(datum[interval.attr]);
            return (
                Number.isFinite(value) &&
                value >= interval.interval_min &&
                value <= interval.interval_max
            );
        });
        datum.selected = selected;
        datum.brushed = [selected];
    });
}
