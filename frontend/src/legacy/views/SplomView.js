...
export default class SplomView {
    ...
    */
        ...
        this.max_attributes = 6;
        this.splom_attributes = splom_attributes.slice(0, this.max_attributes);
        this.focused_attributes = undefined;
        this.rendered_attributes = [];
        this.last_predicates = [];
        this.predicate_mode = model?.get("predicate_mode") ?? "data extent";
    }
    ...
    redraw_kde(mode = "selection") {
        if (this.splom_obj === undefined || this.data.length === 0) {
            return;
        }
        let style = get_point_style(mode);
        let sc = (d, i) => style(d, i).fill;
        let n_brushes = Array.isArray(this.data[0].brushed)
            ? this.data[0].brushed.length
            : 0;
    ...
    draw(splom_attributes = [], predicates = []) {
        if (Array.isArray(this.focused_attributes)) {
            splom_attributes = this.focused_attributes;
        } else if (splom_attributes.length == 0) {
            splom_attributes = this.splom_attributes;
        }
        this.last_predicates = predicates;
        if (
            splom_attributes.length !== this.rendered_attributes.length ||
            splom_attributes.some(
                (attribute, index) => attribute !== this.rendered_attributes[index],
            )
        ) {
            this.splom_obj = undefined;
        }
        // let { n_boxes, predicates } = projection_view.value;
        let n_boxes = predicates.length;

        let color_mode = "selection";
        ...
        if (this.splom_obj === undefined) {
            ...
            this.rendered_attributes = splom_attributes.slice();
    ...
    focus_attributes(attributes) {
        let next_attributes = Array.from(new Set(attributes))
            .filter((attribute) =>
                this.data.some((record) => Number.isFinite(Number(record[attribute]))),
            )
            .slice(0, this.max_attributes);
        if (next_attributes.length < 2) {
            return;
        }
        if (
            Array.isArray(this.focused_attributes) &&
            next_attributes.length === this.focused_attributes.length &&
            next_attributes.every(
                (attribute, index) => attribute === this.focused_attributes[index],
            )
        ) {
            return;
        }

        this.focused_attributes = next_attributes;
        this.splom_obj = undefined;
        this.draw(next_attributes, this.last_predicates);
    }

    clear_attribute_focus() {
        if (!Array.isArray(this.focused_attributes)) {
            return;
        }
        this.focused_attributes = undefined;
        this.splom_obj = undefined;
        this.draw(this.splom_attributes, this.last_predicates);
    }
