...
export function scatter_gl(
    ...
    let current_sc = scales.sc || ((d) => C[0]);
    let current_depth = depth;
    let sc_gl = (d, i) => {
        let c = current_sc(d, i);
    ...
    res.base_scales = {sx: sx_base.copy(), sy: sy_base.copy()};
    res.clear = () => {
    ...
    res.current_transform = d3.zoomIdentity;

    function sync_gl_scales() {
        sx_gl.domain(sx.domain()).range([
            sx_svg2gl(sx.range()[0]),
            sx_svg2gl(sx.range()[1]),
        ]);
        sy_gl.domain(sy.domain()).range([
            sy_svg2gl(sy.range()[0]),
            sy_svg2gl(sy.range()[1]),
        ]);
    }

    function sync_axes() {
        underlay.gx.call(underlay.ax.scale(sx));
        underlay.gy.call(underlay.ay.scale(sy));
        underlay.selectAll(".tick > line").style(
            "stroke",
            stroke || d3.color(background).brighter(),
        );
        underlay.selectAll("path.domain").style("display", "none");
    }

    function update_view(next_sx, next_sy, transform = res.current_transform) {
        sx.domain(next_sx.domain()).range(next_sx.range());
        sy.domain(next_sy.domain()).range(next_sy.range());
        sync_gl_scales();
        sync_axes();
        res.current_transform = transform;
        res.clear();
        res.redraw(data);
    }
    ...
    res.redraw = (next_data = data) => {
        ...
        res.render({
            ...
            depth: data.map((d, i) => current_depth(d, i)),
            stroke: color2gl(stroke),
            stroke_width,
        });
    };

    res.recolor = (new_sc, {depth: next_depth} = {}) => {
        // update color scale
        current_sc = new_sc;
        if (next_depth !== undefined) {
    ...
    res.set_view = (next_sx, next_sy) => {
        update_view(next_sx, next_sy);
    };

    res.reset_zoom = () => {
        if (res.zoom_behavior !== undefined) {
            res.overlay
                .transition()
                .duration(180)
                .call(res.zoom_behavior.transform, d3.zoomIdentity);
            return;
        }
        update_view(sx_base.copy(), sy_base.copy(), d3.zoomIdentity);
    };

    if (zoom) {
        let zoom_selection = overlay1.style("touch-action", "none");
        let zoom_behavior = d3
            .zoom()
            ...
            .on("zoom", (event) => {
                let next_sx = event.transform.rescaleX(sx_base.copy());
                let next_sy = event.transform.rescaleY(sy_base.copy());
                update_view(next_sx, next_sy, event.transform);
            });

        zoom_selection.call(zoom_behavior);
        zoom_selection.on("dblclick.zoom", null);
        res.zoom_behavior = zoom_behavior;
        res.zoom_in = (factor = 1.35) => {
            res.overlay
                .transition()
                .duration(160)
                .call(zoom_behavior.scaleBy, factor, [
                    width / 2,
                    height / 2,
                ]);
        };
        res.zoom_out = (factor = 1.35) => {
            res.overlay
                .transition()
                .duration(160)
                .call(zoom_behavior.scaleBy, 1 / factor, [
                    width / 2,
                    height / 2,
                ]);
        };
